...
// #588: skipTlsVerify=true with a registryUrl that new URL() cannot parse must
// be REJECTED (fail closed), not silently let through -- the guard's whole job
// is to gate this dangerous configuration before it can take effect, so an
// input the guard cannot even classify must never fall through to
// skipTlsVerify actually being honored. The publisher and http transport are
// stubbed so no real network call is made; the rejection must happen before
// either is ever invoked.
...
tr.setInput('registryUrl', 'not-a-valid-url');
