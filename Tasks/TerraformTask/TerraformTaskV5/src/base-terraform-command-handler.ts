 ...
 * ({@link TempFileManager}), argv/flag building ({@link ArgumentBuilder}) and
 * command execution ({@link CommandExecutor}). Still here, and still separable:
 * the per-command implementations, provider-detection output parsing, and
 * plan-result inspection. Each step is a pure refactor with no behavior change.
 */
