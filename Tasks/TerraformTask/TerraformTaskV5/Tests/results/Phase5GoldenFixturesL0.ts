...
import { FIXTURES, FIXTURES_DIR, serializeFixture } from './phase5-golden-fixtures';

// PHASE 5 REGRESSION FOUNDATION (§12.3) + NO-LEAK TRIPWIRE (§12.4.1) for the
// state-inventory digest and the destroy marker.
//
// Each fixture is a SANITIZED synthetic capture (only FAKE secret-looking
// literals, never a real secret) under Tests/fixtures/. For each:
...
//   * every planted-secret literal in the INPUT is asserted to appear NOWHERE in
//     the serialized state digest (the generic catch-all that trips even for a
//     value shape no targeted test covers).
//
// The digest builders are deterministic (redact.ts §2.6), so these goldens are
// byte-stable.

describe('phase 5 golden fixtures (§12.3 regression) + no-leak tripwire (§12.4.1)', () => {
  for (const spec of FIXTURES) {
    describe(spec.input, () => {
      ...
      it('leaks no planted-secret literal into the serialized digest', () => {
