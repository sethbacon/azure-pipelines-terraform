...
describe('OCI backend cache cleanup — opt-in scrub of .terraform/terraform.tfstate (#675)', function () {
    ...
    it('does not register cleanup from handleProvider when backendOCIConfigGenerate is no (native/checked-in backend.tf, not generated by this task)', async () => {
        installInputs(true);
        t.getInput = (name: string) => {
            if (name === 'workingDirectory') return scratchDir;
        ...
        assert.ok(fs.existsSync(cachePath), 'must not assume a PAR-bearing cache exists when this task did not generate the backend config');
    });
});
