...
const testDir = path.join(os.tmpdir(), 'tpc-sentinel-override');
fs.rmSync(testDir, { recursive: true, force: true });
const policyDir = path.join(testDir, 'policies');
fs.mkdirSync(policyDir, { recursive: true });
fs.writeFileSync(path.join(policyDir, 'cost.sentinel'), 'main = rule { false }\n');
...
tr.setInput('overrideSoftMandatory', 'true');
...
const a: ma.TaskLibAnswers = {
    ...
    exec: {
        // exit 1 = policy fail; soft-mandatory with override only warns
        [`${sentinelPath} apply`]: { code: 1, stdout: 'FAIL - cost.sentinel\n' }
    }
};
tr.setAnswers(a);
tr.run();
