...
describe('TerraformInstaller Test Suite', function () {
    ...
    it('cache hit verify pass, forceOnlineReverification=true, TOFU: the separate tofu call site is also covered', async () => {
        const tp = path.join(__dirname, 'CacheHitVerifyPassForcedTofu.js');
        const tr: ttm.MockTestRunner = new ttm.MockTestRunner(tp);
        await tr.runAsync();
