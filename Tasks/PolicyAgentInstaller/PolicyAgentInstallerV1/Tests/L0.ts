...
describe('PolicyAgentInstaller Test Suite', function () {
  ...
  it('cache hit, no marker, source unreachable, requireOnlineReverification=true: fails closed instead of degrading (#778)', async () => {
    ...
    it("registryUrl's OWN host is authorized BEFORE the metadata fetch for a SPECIFIC version, not only on the 'latest' resolution branch (#1104/20)", async () => {
        const tp = path.join(__dirname, 'OpaRegistryUrlHostAuthorizedBeforeMetadataFetch.js');
        const tr: ttm.MockTestRunner = new ttm.MockTestRunner(tp);
        await tr.runAsync();

        runValidations(() => {
            assert(tr.failed, 'task should have failed');
            // M2 mutation-coverage gap: the mocked dns.lookup returns the metadata
    ...
    it('registry default path redirects to a DNS name resolving to a private/metadata address: should reject the redirect hop (#769)', async () => {
        ...
        const tp = path.join(__dirname, 'OpaRegistryDefaultPathRedirectDnsResolvesPrivate.js');
        const tr: ttm.MockTestRunner = new ttm.MockTestRunner(tp);
        await tr.runAsync();
