...
**In scope, late phase (§11 Phase 5) — DELIVERED**
- Structured rendering of `destroy` and `state`. `destroy` reuses the plan digest unchanged (a destroy plan is a plan whose `resource_changes` are all deletes) with an optional `PlanDigest.planMode: "destroy"` presentation marker (`Tasks/TerraformTask/TerraformTaskV5/src/results/plan-digest.ts`, `PlanDigestOptions.mode`); `state` is a new lightweight `StateDigest` inventory (`Tasks/TerraformTask/TerraformTaskV5/src/results/state-digest.ts:52` `buildStateDigest()`, walking `values.root_module` resources, no change actions) rendered in a State pivot (`src/tab/components/StateInventory.tsx`, wired in `src/tab/tabContent.tsx`). Wiring: `base-terraform-command-handler.ts` `destroy()` (~L848, `publishPlanSummary`-gated `-out` + `mode:'destroy'`) and `show()` (~L423, `publishStateResults`-gated, skipped when `commandOptions` shows a saved plan file instead of current state — `hasPositionalCommandArg()`, ~L81). New `task.json` inputs: `publishPlanSummary` `visibleRule` extended to `command = plan || command = destroy` (~L198); `publishStateResults`, new, `visibleRule: command = show` (~L202-206). **`import` was decided OUT OF SCOPE — see below**, not deferred/TBD.
...
5. **Structured `destroy`/`state`/`import` (in scope, late) — DELIVERED for `destroy`/`state`; `import` decided OUT OF SCOPE.** `destroy` reuses `PlanDigest` from a destroy plan file (`terraform destroy` with `publishPlanSummary` set adds `-out=<tempfile>` → `show -json`, same as `plan`) tagged with the optional `planMode: "destroy"` marker so the tab labels it — no new digest type. `state` is a new lightweight `StateDigest` (resource inventory from `show -json` of the current state: `values.root_module.resources[]`, recursing `child_modules[]`, with `sensitive_values` redacted, no action column) built by `state-digest.ts` and rendered as a State pivot (`StateInventory.tsx`). Same redaction, caps, XSS, and testing rules apply; the state inventory has its own golden fixtures (`Tests/fixtures/state-*.json` + `.expected.json`) and no-leak tripwire coverage (`Tests/results/StateDigestL0.ts`, `Phase5GoldenFixturesL0.ts`).
   **`import` decision (resolved, not deferred):** a standalone `terraform import` emits little machine-readable JSON of value, and Terraform 1.5+ `import {}` blocks already surface as create/import actions inside the **plan** digest — so a dedicated import digest would add schema surface for marginal benefit. Phase 5 ships **no** `import` digest type or pivot; standalone `terraform import` continues on the raw ANSI fallback view, unchanged. See `docs/design/plan-apply-digest-spec.md` §7.5 for the normative record of this decision.
