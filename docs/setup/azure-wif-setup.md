...
- **Default (`false`) — ID token refresh mode:** the task exports the broad ADO pipeline OIDC access token (`ARM_OIDC_REQUEST_TOKEN`) into the Terraform process environment for the **whole run**, and the `azurerm` provider/backend refreshes it as needed. It also pins `ARM_OIDC_REQUEST_URL` to the job's OIDC endpoint, validated against the same Azure DevOps host allowlist the task applies to its own token requests — the provider previously consumed the agent's ambient `SYSTEM_OIDCREQUESTURI` unvalidated, and that name is now cleared from the child environment (#1026). This is present regardless of whether `runAzLogin` is enabled — `runAzLogin` only additionally signs the Azure CLI in with it. Because the token sits in the process environment for the run's duration, it's inherited by any `local-exec` provisioner or `external` data source the configuration runs.
