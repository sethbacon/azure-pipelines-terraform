...
**Phase 5 extended v1 additively** — added the `StateDigest` union member, the optional `PlanDigest.planMode` destroy marker, and factored the `DigestMeta` / `OutputValue` shared types — **without** bumping `schemaVersion`, because the feature is still unreleased and nothing external consumes the version yet (§7). Existing plan/apply digests are byte-unchanged; the new members are purely additive, so a v1 consumer that predates Phase 5 ignores what it does not recognize.
