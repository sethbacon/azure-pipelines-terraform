...
1. **Provider-echoed secrets in diagnostics.** Apply/plan diagnostics (`diagnostics[].summary`/`.detail`) are freeform strings a provider can construct from user or resource input, and there is no structured sensitivity mask for them. The task scrubs known secrets it registered via `setSecret()` and applies a best-effort high-entropy/format heuristic (long base64/hex runs, PEM blocks), but a provider-authored message that embeds a secret the task never registered — and that doesn't match the heuristic — can still appear in a diagnostic. Mitigated by `includeDiagnosticDetail` (default `false`): only the shorter `summary` field is included by default, and the longer, more leak-prone `detail` field requires an explicit opt-in.
