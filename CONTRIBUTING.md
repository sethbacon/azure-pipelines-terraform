...
### Real-terraform smoke harness (TerraformTaskV5)

TerraformTaskV5 also ships a **real-terraform behavioral smoke harness**, separate from the mock-based `npm test` above:
...
This compiles the task and runs `mocha --timeout 60000 --require ts-node/register Tests/SmokeL0.ts` against a real `terraform` binary (1.15.7 in CI) on a local backend — no cloud, no mocking. It requires `terraform` on `PATH`. In CI this runs as its own required gate, **`Build and Test V5 Smoke`** (`ubuntu-latest` and `windows-2025`), kept separate from the mock `npm test` job so that one stays terraform-free. See [docs/initiatives/smoke-fuzz-testing-plan.md](docs/initiatives/smoke-fuzz-testing-plan.md) for the harness's design and the still-planned property/fuzz test layer.
...
- the OCI UPST exchange, now `exchangeOidcForUpst()` in `@4cloudguru/pipeline-task-ado` (#1074) — retried inside the package, not here
...
- The tab uses `dangerouslySetInnerHTML` only to render ANSI-converted HTML for legacy `terraform-plan-results` output (`RawView`'s `"ansi"` format); structured digests, including their raw views, render as text nodes. Any change to `ansiToHtml` must keep opening/closing `<span>` tags balanced. A state-machine rewrite of the converter is the planned hardening here.
...
- `src/tab/` has a Jest harness (`jest.config.js` at the repo root) — run it with `npm run test:tab`. It covers `ansi-to-html.test.ts` and `tabContent.test.tsx` (loading/error/empty states, multi-plan select, oversize-plan download, reloads, `loadPlans` edge cases) and enforces coverage thresholds (statements 80%, branches 78%, functions 60%, lines 80%; see `jest.config.js` for the rationale behind those numbers). Tests run in Jest's `node` environment and render with `react-dom/server`; one that needs a live DOM (re-renders, `<details>` toggle events, clicks) opts into jsdom with a `@jest-environment jsdom` docblock at the top of its file, as `tabContent.dom.test.tsx` and `components/RawView.dom.test.tsx` do. Run it alongside `npm run webpack` when changing the tab. It does not cover the module-level SDK bootstrap block (`SDK.ready().then(...)`) — that needs a real DOM/ADO iframe, so end-to-end verification of that wiring still requires a private publish.
...
3. The per-task `Minor` bumps happen **automatically** on the Release PR. ADO agents cache tasks by `Major.Minor` and will not pick up new code until `Minor` increments, so every task whose `src/` changed OR whose `task.json` itself changed (e.g. a `defaultValue` edit with no accompanying `src/` change) since the last release must have its `task.json` `Minor` incremented before the release is tagged. Three layers enforce this, so in the normal case there is nothing to do by hand:
   ...
   **security** issue in at least one of the release's PRs **must** have its `Minor` bumped in
   ...
   automation bumps any task whose `src/` OR `task.json` changed (#676), which already covers
