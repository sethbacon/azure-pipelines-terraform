...
The publish itself goes through the `publish-marketplace` composite action (`4cloudguru/shared-workflows/.github/actions/publish-marketplace`), which keeps the minted Entra token on tfx's stdin (never on argv, where it would be readable in `/proc/<pid>/cmdline`) and retries a transient upstream failure a bounded number of times — the sibling `azure-pipelines-packer` repo lost its v1.2.7 release to a single HTTP 503 at this step, leaving an orphaned draft release behind. A deterministic rejection is never retried. It runs as a step inside the `marketplace` environment job, so it neither changes nor bypasses that environment's gates. Formerly a byte-identical copy of the same script in each of this repo, `azure-pipelines-packer` and `azure-pipelines-release-docs`; centralized after that duplication let a real regression (v1.15.2's stuck release) ship in one repo and sit unfired in the other two. The pre-publish cosign integrity gate (`verify-vsix-signature`, same shared repo) is centralized the same way and for the same reason — comparing the three repos' publish jobs line by line while doing so found `azure-pipelines-packer` had never had that gate at all.
...
BOTH repositories — the copies have already diverged once (#1046: the egress-authorization gate
plus three `src/` files), which is what let fourteen credential-bearing ServiceNow egress sites go
unadjudicated here while the same code was adjudicated in the other repo. That gate is no longer a
copy anywhere: it is the shared `check-egress-authorization` composite action, adopted here at
v1.28.0, so this particular divergence cannot recur.
...
| `base-terraform-command-handler.ts`    | Abstract base with shared command implementations. **~1,390 lines by decision, not by neglect (#878, closed):** the temp-file, argv, execution and results-publishing collaborators were extracted (2302 -> ~1390); the remaining two thirds are the sixteen per-command implementations, and splitting those out would require making ~10 `protected` members **public** -- widening the API of the class that owns credential handling and auth-scheme validation. Declined in favour of the narrower surface; re-open that decision before finishing the split |
...
The `Check Shared Module Parity` job also runs seven **shared composite actions** from `4cloudguru/shared-workflows`, each pinned by full commit SHA:
...
- `check-egress-authorization` — the SSRF class (#161 / #188 / #191 / #200 / #201): an egress destination must be authorized by its RESOLVED address rather than its textual form, and re-authorized on every redirect hop. The last of the seven to move, and the only one whose hand-copies disagreed rather than one simply being behind — the union of them landed here first (#1173), and the composite ships exactly those bytes.
- `check-docs-claims` — the documented-claims class, described below.

Six of the seven declare a **measured floor** (`min-sites` / `min-scanned` / `min-cells` / `min-claims`) in `.github/workflows/unit-test.yml` with the date and the producing command beside it, because each of those six exits 0 over a repository it enumerated nothing in; the floor is what tells "looked and found none" apart from "looked nowhere". `check-shared-module-pins` and `check-docs-claims` were the two that reported their own denominators without refusing an empty one (4cloudguru/shared-workflows#75); v1.27.0 closed that, and both floors became REQUIRED inputs, so an omitted one now fails the step rather than inheriting a permissive default. Only `check-enforced-disciplines` takes no floor: it fails outright on a repository with no `Tasks/` tree. No new required status check was introduced — each of these is a **step inside an existing required job**, so branch protection needs no change.
...
That sibling checkout is no longer only a convenience for running a gate by hand: `npm test` for `TerraformTaskV5`, `TerraformInstallerV1`, `PolicyAgentInstallerV1` and `TerraformDocsInstallerV1` now needs it too. `Tests/ProxyParityL0.ts`, `Tests/CredentialFailClosedMatrixL0.ts`, `Tests/ArtifactTrustL0.ts` and `Tests/EgressAuthorizationL0.ts` **spawn** their gate and assert its whole enumerated set, and `Tests/shared-gate.ts` (byte-identical in all four tasks, gated by `scripts/check-shared-modules.js`, and copied *from* `azure-pipelines-packer`, whose copy landed first: the provenance header names it and the replay's cross-repo-copy-parity signature compares the two) is what finds it: the composite's exported `github.action_path` on a runner, `../shared-workflows` beside this checkout for a developer, and a throw naming the `git clone` to run otherwise. It never skips — the assertions it feeds are the only thing enumerating those classes under `npm test`, so a could-not-run that read like a clean run would be the failure this estate keeps re-learning — and every suite prints one `[shared-gate] <file> sha256:… <- <path> (via …)` line so which bytes ran is answerable from a log.
