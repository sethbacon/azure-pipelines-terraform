...
| `requireCosignVerification` | `true`      | For OpenTofu (`binary=tofu`): fail if cosign/Sigstore verification cannot be performed. With the default `cosignSource=managed` no agent preparation is needed — the task installs its own cosign.                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `cosignSource`              | `managed`   | For OpenTofu (`binary=tofu`): where the `cosign` binary comes from. `managed` (default) downloads the **pinned** sigstore/cosign release for the agent's platform from `github.com`, verifies it against a SHA256 shipped in the task, caches it, and runs only that copy — the agent's `PATH` is never consulted, and a download failure with `requireCosignVerification=true` fails the task rather than falling back. `ambient` keeps the historical PATH lookup for image-baked or air-gapped agents that provision cosign themselves; that binary is unverified unless you also set `cosignSha256`, and the task emits a warning saying so. |
| `cosignSha256`              | —           | Optional pin for the `cosign` binary itself, honoured in both `cosignSource` modes. Required in practice only under `ambient`, where nothing else establishes the verifier's identity.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| `allowDirectFallback`   | `true`  | **Security-sensitive.** Allow Terraform to fall back to direct registry download if the mirror doesn't have a provider. Operators relying on this mirror for air-gapped/compliance isolation must explicitly set this to `false` — the default permits direct internet egress for any provider the mirror lacks. |
| `directExcludePatterns` | —       | Provider patterns to exclude from direct download (one per line). Forces these through the mirror.                                                                                                                                                                                                               |
| `directIncludePatterns` | —       | Provider patterns to include for direct download (one per line). Only these can bypass the mirror. By itself this does not stop the mirror being consulted for them — pair it with `mirrorExcludePatterns` to actually bypass the mirror.                                                                        |
| `mirrorExcludePatterns` | —       | Provider patterns to exclude from the network mirror (one per line). Terraform consults every installation method whose patterns match a provider, so this — not `directIncludePatterns` alone — is what stops the mirror being consulted for a given provider.                                                  |
| `mirrorIncludePatterns` | —       | Provider patterns to include for the network mirror (one per line). Only these are fetched from the mirror; all others bypass it.                                                                                                                                                                                |
...
| `publishPlanSummary`      | `plan`, `destroy`                                 | —         | Name for a **structured, redacted** JSON summary of the plan (resource changes, outputs, drift) published to the **Terraform** results tab's Plan pivot. Leave empty to disable. Adds `-out=<tempfile>` to the plan and runs `terraform show -json` on it to build the summary — unless your `commandOptions` already saves the plan with its own `-out=<path>`, in which case that path is reused (no second `-out=` is injected); every sensitive value renders as `(sensitive)`. On `destroy`, the summary is built from the destroy's own plan the same way (Terraform computes and saves one before applying) and the tab labels it **Destroy**; destroy still auto-approves and still fails the task on a non-zero exit. Redaction relies on Terraform correctly marking values `sensitive` (`after_sensitive`/`sensitive_values`) — see [SECURITY.md](SECURITY.md). |
...
| `awsSessionName`                           | AWS      | _derived per run_    | Assumed-role session name. Blank derives `ado-tf-<TeamProject>-<BuildId>` so CloudTrail can attribute a change to a run; see [aws-wif-setup.md](docs/setup/aws-wif-setup.md#optional-pinning-the-role-session-name).                          |
...
| `gcpProjectId`                             | GCP      | —                    | Project ID the Google provider defaults to (`GOOGLE_PROJECT`) under WIF, matching the service-connection scheme; when empty the project NUMBER is used instead.
...
| -------------------------------------------- | ------- | -------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `backendServiceArm`                          | azurerm | — (required)         | Azure Resource Manager service connection for the backend. On a state command with `provider: azurerm`, this must be the same connection as `environmentServiceNameAzureRM` unless the backend was bound to its own credential at `init` — see [separate backend and provider service connections](docs/yaml-examples.md#separate-backend-and-provider-service-connections-same-cloud). |
| `backendAzureRmUseEntraIdForAuthentication`  | azurerm | `true`               | Sets Terraform's `use_azuread_auth` for the azurerm backend.                                                                                                                                                                                                                                                         |
| `backendAzureRmUseCliFlagsForAuthentication` | azurerm | `false`              | Use CLI flags (`client_id`, `ado_pipeline_service_connection_id`, `use_oidc`) so the backend can use a separate service connection from the provider. Set this on the `init` step; it is what makes a separate `backendServiceArm` work on later state commands.                                                      |
...
| `backendAWSSessionName`                      | s3      | _derived per run_    | Assumed-role session name. Blank derives `ado-tf-backend-<TeamProject>-<BuildId>`; see [aws-wif-setup.md](docs/setup/aws-wif-setup.md#optional-pinning-the-role-session-name).                                                                                                                                       |
...
| `skipTlsVerify`   | `false`                    | **Security-sensitive.** Disables TLS certificate validation for the private-registry connection while the `apiKey` bearer is transmitted — with verification off, `apiKey` is a bearer credential capturable by anyone on-path between the agent and the registry. Prefer installing the private CA into the agent's trust store via `NODE_EXTRA_CA_CERTS` instead of disabling verification; use `skipTlsVerify` only when that isn't possible for an internal registry behind a private CA the agent does not trust. It is honoured **only** when `registryUrl` is, or resolves to, a private/link-local address — a public destination, a URL that does not parse, or one carrying `user:password@` credentials is refused (see [SECURITY.md](SECURITY.md#optional-feature-residual-risk-the-tls-verification-opt-outs-skiptlsverify-rejectunauthorized-false)). |
...
| `rejectUnauthorized`      | `true`                            | Verify the callback endpoint's TLS certificate (leave enabled in production). If the endpoint uses a private CA, prefer installing that CA into the agent's trust store via `NODE_EXTRA_CA_CERTS` over disabling verification — with verification off, `callbackToken` is a bearer credential capturable by anyone on-path between the agent and TSM. Setting it `false` is honoured **only** when `callbackUrl` is, or resolves to, a private/link-local address — a public destination, a URL that does not parse, or one carrying `user:password@` credentials is refused (see [SECURITY.md](SECURITY.md#optional-feature-residual-risk-the-tls-verification-opt-outs-skiptlsverify-rejectunauthorized-false)). |
